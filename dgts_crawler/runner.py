...
def run_crawl(
    ...
    _emit(progress, f"Output: {config.output_path}")
...
def _record_history(
    ...
    result = HistoryStore(config.history_db_path).record_crawl(
        notice_kind=notice_kind,
        start_date=start_date,
        end_date=end_date,
        snapshots=snapshots,
    ...
    missing_note = "" if detect_missing else " (crawl dừng sớm nên không kiểm tra MISSING)"
    _emit(
        progress,
        "Lịch sử: "
...
def _missing_exists_validator(
    ...
    def validator(snapshot: HistorySnapshot) -> bool:
        try:
            return _missing_detail_exists(notice_kind, client, snapshot.notice_id)
